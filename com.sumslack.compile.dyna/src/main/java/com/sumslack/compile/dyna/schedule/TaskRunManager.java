...
public class TaskRunManager {
	 ...
	 */
	public static void runTaskJob(TaskJob taskJob) throws Exception {

		try {
			taskJob.start();
			THREAD_POOL.put(taskJob.getName(), taskJob);
		} catch (Exception e) {
			throw new Exception("the thread " + taskJob.getName() + e.toString());
		}
	}

	public static synchronized boolean stop(String key) throws Exception {
		if (THREAD_POOL.containsKey(key)) {
			...
			if (remove.isOver()) {
				...
				throw new Exception(key + " stop Failure");
			}
		} else {
